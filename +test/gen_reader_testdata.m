...
% A classdef object is stored as its struct() contents, with the real class in
% `t` and as:"struct" beside it. It cannot be a test.json_cases entry because
% it deliberately decodes back to a plain struct, so its payload is added here
% so the readers' object branch is covered by generated data too.
recs{end + 1} = struct( ...
    'name',    'classdef_object', ...
    'class',   'test.json_testobj', ...
    'dims',    [1 1], ...
    'payload', json.mdumps(test.json_testobj(7)));
