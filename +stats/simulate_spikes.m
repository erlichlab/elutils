...
if nargin==0 % Part of help!
    ...
    n_trials = 150;
    trial_starts = linspace(0,n_trials*10, n_trials )';
    event_ts = [trial_starts trial_starts + rand(size(trial_starts))+0.1];
    event_weights = randi([-1 2], n_trials, numel(kernel))*3;
    [spktimes, rate_function] = stats.simulate_spikes(event_ts, kernel, event_weights,'krn_bin_size',krn_bin_size,'baseline',5);
    clrs = {[0 0.75 0], [1 0.5 0], [0 0.75 0]/1.5, [1 0.5 0]/1.5};
    figure(314); clf; draw.exampleraster(event_ts(:,1), spktimes,'cnd',event_weights(:,1),'errorbars',0,'clrs',clrs);
